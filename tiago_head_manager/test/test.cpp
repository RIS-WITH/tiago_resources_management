...
std::vector<std::string> reactive_input_names = {"human_monitoring,", "environment_monitoring,acting,speaking,proximity"};
...
public:
    ...
    RosNodeFixture(){
        ...
        for(auto &n : reactive_input_names){
            reactive_input_publishers.emplace_back(new MessageGenerator<::tiago_head_manager_msgs::PitchYaw>(nh,"/tiago_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::tiago_head_manager_msgs::Point>(nh,"/tiago_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::tiago_head_manager_msgs::PrioritizedPitch>(nh,"/tiago_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::tiago_head_manager_msgs::PrioritizedYaw>(nh,"/tiago_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::tiago_head_manager_msgs::PrioritizedTrajectory>(nh,"/tiago_head_manager",n));
        }
        set_priorities = nh.advertise<resource_management_msgs::PrioritiesSetter>("/tiago_head_manager/set_priorities",100,/*latch=*/true);
        ...
        active_buffer = nh.subscribe<std_msgs::String>("/tiago_head_manager/active_buffer",10,cb);
...
public:
    ...
    {
        state_machine_client = nh.serviceClient<tiago_head_manager_msgs::StateMachineRegister>("/tiago_head_manager/state_machines_register");
    }

    tiago_head_manager_msgs::StateMachineRegister makeStateMachine(std::string initial, double timeout, double dl_in_secs_from_now, int prio){
        tiago_head_manager_msgs::StateMachineRegister sig;
    ...
    void addState(tiago_head_manager_msgs::StateMachineRegister::Request &request, const std::string &id, const std::vector<::resource_management_msgs::StateMachineTransition> & transitions = {}){
        tiago_head_manager_msgs::StateMachineStatePitchYaw state;
        state.header.id=id;
        state.header.transitions = transitions;
        request.state_machine.states_PitchYaw.push_back(state);
    }
    ...
    tiago_head_manager_msgs::StateMachineRegister makeSimpleStateMachine(int prio){
...
int main(int argc, char *argv[]){
    ...
    ros::init(argc,argv,"tests_tiago_head_manager");
    ros::NodeHandle nh;
